...
# --- ДОБАВЛЕНО: Скрываем стрелки +/- у полей ввода чисел ---
st.markdown("""
<style>
...
def init_db():
    ...
    # --- ИСПРАВЛЕНО: Добавлено ON DELETE SET NULL для безопасного удаления групп ---
...
if choice == "Клиенты и Группы":
    ...
    # --- ИСПРАВЛЕНО: Убрана кнопка, оставлен свернутый экспандер групп всегда на странице ---
            ...
            with st.form("add_group"):
                ...
                if st.form_submit_button("Добавить группу"):
                    if new_group:
                        ...
                        st.rerun()
        with col2:
            st.write("### Редактирование и удаление групп")
            groups_df = run_query("SELECT * FROM groups", fetch=True)
            if not groups_df.empty:
                selected_group = st.selectbox("Выберите группу", groups_df['id'], format_func=lambda x: groups_df[groups_df['id']==x]['name'].iloc[0])
    ...
    if not clients_df.empty:
        ...
        # --- ИСПРАВЛЕНО: Глобальная сортировка по всем данным ---
        # Инициализируем состояние сортировки
        if "clients_sort" not in st.session_state:
            st.session_state.clients_sort = ("id", "desc")
        
        # Применяем сортировку ко всей таблице
        sort_col, sort_dir = st.session_state.clients_sort
        clients_df = clients_df.sort_values(by=sort_col, ascending=(sort_dir == "asc"))
        ...
        # Отображаем таблицу с обработкой сортировки
        def handle_sort(sort_info):
            if sort_info:
                st.session_state.clients_sort = (sort_info.column, sort_info.direction)
                st.rerun()
        
        st.dataframe(page_df, use_container_width=True, on_sort=handle_sort)
...
# --- 3. ЗАКАЗЫ ---
    ...
    if not df_orders.empty:
        ...
        # --- ИСПРАВЛЕНО: Статистика за текущий месяц, а не за 30 дней ---
        col1, col2, col3, col4 = st.columns(4)
        current_month_stats = run_query('''
            SELECT 
            ...
            FROM orders 
            WHERE execution_date >= date("now", "start of month")
        ''', fetch=True).iloc[0]
        
        with col1:
            st.metric("Всего заказов за месяц", current_month_stats['total_orders'])
        with col2:
            st.metric("Общая сумма за месяц", f"{int(current_month_stats['total_sum']):,.0f} ₽".replace(",", " "))
        with col3:
            st.metric("Средний чек за месяц", f"{int(current_month_stats['avg_check']):,.0f} ₽".replace(",", " "))
        with col4:
            st.metric("В работе за месяц", current_month_stats['in_work'])
        
        # --- ИСПРАВЛЕНО: Глобальная сортировка для заказов ---
        if "orders_sort" not in st.session_state:
            st.session_state.orders_sort = ("execution_date", "desc")
        
        sort_col, sort_dir = st.session_state.orders_sort
        df_orders = df_orders.sort_values(by=sort_col, ascending=(sort_dir == "asc"))
        ...
        def handle_order_sort(sort_info):
            if sort_info:
                st.session_state.orders_sort = (sort_info.column, sort_info.direction)
                st.rerun()
        
        st.dataframe(page_df, use_container_width=True, on_sort=handle_order_sort)
        
        # --- ИСПРАВЛЕНО: Исправлена ошибка с отображением неверных данных при редактировании заказов ---
        ...
        if edit_id > 0:
            ...
            if not order_data.empty:
                # Добавлен уникальный ключ для экспандера, чтобы сбросить состояние при смене ID
                with st.expander(f"Редактирование заказа #{edit_id}", key=f"order_edit_{edit_id}"):
...
# --- 4. ДЕТАЛИЗАЦИЯ ЗАКАЗА ---
    ...
    if not orders_df.empty:
        orders_df['label'] = orders_df.apply(lambda x: f"Заказ #{x['id']} - {x['name']} ({format_date(x['execution_date'])})", axis=1)
        ...
        with col1:
            ...
            with st.form("add_item_form"):
                ...
                # --- ИСПРАВЛЕНО: Убраны стрелки +/- и исправлен формат отображения ---
                i_amount = st.number_input("Сумма", min_value=0.0, step=100.0, value=0.0, format="%.0f")
        ...
        with col2:
            ...
            if not items_df.empty:
                # --- ИСПРАВЛЕНО: Форматирование даты и суммы по требованиям ---
...
# --- 5. ОТЧЁТЫ ---
    ...
    if not df.empty:
        ...
        # --- ИСПРАВЛЕНО: Отчет 7 с группировкой по имени и дате и суммированием суммы ---
        st.subheader("7. Оплаты за последнюю неделю")
        df_7 = run_query('''
            SELECT c.name, oi.payment_date, SUM(oi.amount) as amount 
            FROM order_items oi
        ...
        if not df_7.empty:
            ...
            df_7['amount'] = df_7['amount'].apply(format_currency)
            st.dataframe(df_7, use_container_width=True)
        else:
            st.write("Нет данных об оплатах за последнюю неделю.")
    else:
