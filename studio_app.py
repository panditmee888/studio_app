...
# ======================= 3. ЗАКАЗЫ И УСЛУГИ (ФИНАЛЬНАЯ ВЕРСИЯ) =======================
    ...
    with col_right:
        ...
        if not all_orders_df.empty:
            ...
            disp_orders['total_amount'] = disp_orders['total_amount'].apply(lambda x: f"{format_currency(x)} ₽" if x else "0 ₽")

            # Кликабельные строки
            for idx, row in disp_orders.iterrows():
                if st.button(
                    f"№{row['id']} | {row['client_name']} | {row['execution_date']} | {row['status']} | {row['total_amount']}",
                    key=f"order_btn_{row['id']}",
                    use_container_width=True
                ):
                    # Автоматически переключаем на редактирование и выбираем клиента + заказ
                    st.session_state.order_mode = "Редактировать"
                    client_name = row['client_name']
                    if client_name in client_map:
                        st.session_state.order_client = client_name
                        st.session_state.sel_existing_order = f"№{row['id']} | {row['execution_date']} | {row['status']}"
                    st.rerun()
        else:
