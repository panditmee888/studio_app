...
# --- УПРАВЛЕНИЕ КЛИЕНТАМИ ---
    ...
    with st.expander("🏷️ Управление группами", expanded=False):
        # ⚡ Загружаем список групп ОДИН раз в начале, чтобы не делать лишних запросов в БД
        groups_df = run_query("SELECT id, name FROM groups ORDER BY id DESC", fetch=True)
    
        # ✅ Делаем надписи действий идентичными с клиентами
        ...
        # ✅ Общая разметка на две колонки для всех действий
        ...
        with col1:
            if action == "Добавить":
                ...
                if groups_df.empty:
                    ...
                    selected_group = st.selectbox("Выберите группу для редактирования", group_options, key="group_select")
                    ...
                    # 🛡️ Проверяем, что группа выбрана, прежде чем обрабатывать
                    if selected_group:
                        ...
                        edited_group = st.data_editor(
                            ...
                            hide_index=True,
                            use_container_width=True,
                            key="single_group_editor"
                        )
    
                if action == "Редактировать" and selected_group is not None and edited_group is not None:
                    # 🚦 Проверяем изменения ТОЛЬКО если группа выбрана и есть что сравнивать
                    if not edited_group.equals(edit_df):
                        new_row = edited_group.iloc[0]
                        new_name = new_row['name'].strip()
                ...
                elif action == "Удалить" and selected_group is not None:
                    if st.button("🗑️ Подтвердить удаление группы"):
                        # 🚦 Проверяем наличие клиентов ТОЛЬКО при нажатии кнопки, а не на каждом рендере
        ...
        # ✅ Список групп всегда справа для всех действий
        with col2:
