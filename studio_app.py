...
# --- 3. ЗАКАЗЫ И УСЛУГИ (ИСПРАВЛЕННАЯ ВЕРСИЯ) ---
    ...
    with col_left:
        ...
        # Переменные для выбранного заказа
        ...
        with col_status:
            status = st.selectbox("🏷️ Статус", STATUS_LIST, key="order_status")

        order_id = None
        # Если выбран режим редактирования/удаления
        if order_mode in ["Редактировать", "Удалить"] and selected_client_name != "— Выберите клиента —":
            ...
            if client_id:
                ...
                if not orders_df.empty:
                    ...
                    # Подгружаем данные выбранного заказа
                    order_data = run_query("SELECT * FROM orders WHERE id=?", (order_id,), fetch=True).iloc[0]
                    execution_date = st.date_input("📅 Дата исполнения", value=datetime.strptime(order_data['execution_date'], "%Y-%m-%d").date(), key="order_date_edit")
                    status = st.selectbox("🏷️ Статус", STATUS_LIST, index=STATUS_LIST.index(order_data['status']), key="order_status_edit")
                else:
                    st.info("У этого клиента пока нет заказов")

        # ✅ Скрываем экспандер в режиме добавления заказа
        if order_mode != "Добавить" and order_id is not None:
            with st.expander("🛠️ Управление услугами в заказе", expanded=True):
                # Сначала показываем состав заказа
                ...
                if service_mode == "Добавить":
                    with st.form("form_add_service", clear_on_submit=True):
                        ...
                        with c2:
                            ...
                            new_hours = st.text_input("⏱️ Часы", value="0.0", key="add_hours")
    ...
    with col_right:
        ...
        if not all_orders_df.empty:
            display_orders = all_orders_df.copy()
            ...
            display_orders.columns = ["ID", "Клиент", "Дата", "Статус", "Итого"]

            # Делаем таблицу интерактивной
            selected_order = st.dataframe(
                display_orders,
                use_container_width=True,
                hide_index=True,
                on_select="rerun",
            ...
            # Автоматическое заполнение формы при выборе заказа
            if selected_order.selection.rows and order_mode in ["Редактировать", "Удалить"]:
                selected_row = display_orders.iloc[selected_order.selection.rows[0]]
                st.session_state["order_client"] = selected_row["Клиент"]
                st.rerun()
        else:
            st.info("📌 Пока нет созданных заказов")
