...
if choice == "Клиенты и Группы":
    ...
    if not clients_df.empty:
        ...
        selected_client = st.selectbox("Выберите клиента для редактирования", [""] + client_options)

        if selected_client:
            client_id = int(selected_client.split(" - ")[0].replace("#", ""))
            client_data = clients_df[clients_df['id'] == client_id].iloc[0]

            # Форма редактирования одной строки
            st.markdown("---")
            st.markdown(f"#### Редактирование клиента #{client_id}")

            # Создаем DataFrame из одной строки для редактирования
            edit_df = pd.DataFrame({
                'ID': [client_data['id']],
                'Имя': [client_data['name']],
                'Пол': [client_data['sex']],
                'Телефон': [client_data['phone']],
                'VK': [client_data['vk_id']],
                'Telegram': [client_data['tg_id']],
                'Группа': [client_data['group_name']],
                'Первая оплата': [format_date_display(client_data['first_order_date'])]
            })

            # Редактируемая таблица из одной строки
            edited_df = st.data_editor(
                edit_df,
                column_config={
                    "ID": st.column_config.NumberColumn("ID", disabled=True),
                    "Имя": st.column_config.TextColumn("Имя"),
                    "Пол": st.column_config.SelectboxColumn("Пол", options=["М", "Ж"]),
                    "Телефон": st.column_config.TextColumn("Телефон"),
                    "VK": st.column_config.TextColumn("VK"),
                    "Telegram": st.column_config.TextColumn("Telegram"),
                    "Группа": st.column_config.SelectboxColumn("Группа", options=["Без группы"] + groups_list),
                    "Первая оплата": st.column_config.TextColumn("Первая оплата")
                },
                hide_index=True,
                use_container_width=True,
                key=f"client_editor_{client_id}"
            )

            # Сохранение изменений
            if st.button("Сохранить изменения", key=f"save_client_{client_id}"):
                group_name = edited_df.iloc[0]['Группа']
                ...
                run_query('''
                    ...
                    WHERE id=?
                ''', (
                    edited_df.iloc[0]['Имя'],
                    edited_df.iloc[0]['Пол'],
                    edited_df.iloc[0]['Телефон'],
                    edited_df.iloc[0]['VK'],
                    edited_df.iloc[0]['Telegram'],
                    g_id,
                    parse_date_to_db(edited_df.iloc[0]['Первая оплата']),
                    client_id
                ))
...
# --- 2. ПРАЙС-ЛИСТ ---
    ...
    if not services_df.empty:
        ...
        selected_service = st.selectbox("Выберите услугу для редактирования", [""] + service_options)

        if selected_service:
            service_id = int(selected_service.split(" - ")[0].replace("#", ""))
            service_data = services_df[services_df['id'] == service_id].iloc[0]

            # Форма редактирования одной строки
            st.markdown("---")
            st.markdown(f"#### Редактирование услуги #{service_id}")

            # Создаем DataFrame из одной строки для редактирования
            edit_df = pd.DataFrame({
                'ID': [service_data['id']],
                'Услуга': [service_data['name']],
                'Мин. прайс': [format_currency(service_data['min_price'])],
                'Описание': [service_data['description'] or ""]
            })
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    "ID": st.column_config.NumberColumn("ID", disabled=True),
                    "Услуга": st.column_config.TextColumn("Услуга"),
                    "Мин. прайс": st.column_config.TextColumn("Мин. прайс ₽"),
                    "Описание": st.column_config.TextColumn("Описание")
                },
                hide_index=True,
                use_container_width=True,
                key=f"service_editor_{service_id}"
            )

            # Сохранение изменений
            if st.button("Сохранить изменения", key=f"save_service_{service_id}"):
                run_query('''
                    UPDATE services_catalog
                    SET name=?, min_price=?, description=?
                    WHERE id=?
                ''', (
                    edited_df.iloc[0]['Услуга'],
                    parse_currency(edited_df.iloc[0]['Мин. прайс']),
                    edited_df.iloc[0]['Описание'],
                    service_id
                ))
                st.success("✅ Изменения сохранены!")
                st.rerun()
...
# --- 3. ЗАКАЗЫ ---
    ...
    if not orders_df.empty:
        ...
        with col2:
            ...
            st.metric("Общая сумма", f"{format_currency(total_sum)} ₽")
        with col3:
        ...
        selected_order = st.selectbox("Выберите заказ для редактирования", [""] + order_options)

        if selected_order:
            order_id = int(selected_order.split(" - ")[0].replace("#", ""))
            order_data = orders_df[orders_df['id'] == order_id].iloc[0]

            # Форма редактирования одной строки
            st.markdown("---")
            st.markdown(f"#### Редактирование заказа #{order_id}")

            # Создаем DataFrame из одной строки для редактирования
            edit_df = pd.DataFrame({
                'ID': [order_data['id']],
                'Клиент': [order_data['client_name']],
                'Дата исполнения': [format_date_display(order_data['execution_date'])],
                'Статус': [order_data['status']],
                'Сумма': [f"{format_currency(order_data['total_amount'])} ₽"]
            })
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    "ID": st.column_config.NumberColumn("ID", disabled=True),
                    "Клиент": st.column_config.SelectboxColumn("Клиент", options=client_names),
                    "Дата исполнения": st.column_config.TextColumn("Дата исполнения"),
                    "Статус": st.column_config.SelectboxColumn("Статус", options=STATUS_LIST),
                    "Сумма": st.column_config.TextColumn("Сумма", disabled=True)
                },
                hide_index=True,
                use_container_width=True,
                key=f"order_editor_{order_id}"
            )

            # Сохранение изменений
            if st.button("Сохранить изменения", key=f"save_order_{order_id}"):
                client_id = client_map.get(edited_df.iloc[0]['Клиент'])
                run_query('''
                    UPDATE orders
                ...
                ''', (
                    client_id,
                    parse_date_to_db(edited_df.iloc[0]['Дата исполнения']),
                    edited_df.iloc[0]['Статус'],
                    order_id
                ))
                st.success("✅ Изменения сохранены!")
                st.rerun()
    ...
    if not orders_df.empty:
        ...
        with col1:
            ...
            with st.form("add_item_form"):
                service_choice = st.selectbox("Услуга (из каталога)", srv_list if srv_list else ["Нет услуг"])
        ...
        with col2:
            ...
            if not items_df.empty:
                ...
                st.info(f"💰 Общая сумма: {format_currency(total_amount)} ₽")

                # Выбор услуги для редактирования
                st.markdown("### ✏️ Редактирование услуги")
                item_options = [f"#{row['id']} - {row['service_name']}" for _, row in items_df.iterrows()]
                selected_item = st.selectbox("Выберите услугу для редактирования", [""] + item_options)

                if selected_item:
                    item_id = int(selected_item.split(" - ")[0].replace("#", ""))
                    item_data = items_df[items_df['id'] == item_id].iloc[0]

                    # Форма редактирования одной строки
                    st.markdown("---")
                    st.markdown(f"#### Редактирование услуги #{item_id}")

                    # Создаем DataFrame из одной строки для редактирования
                    edit_df = pd.DataFrame({
                        'ID': [item_data['id']],
                        'Услуга': [item_data['service_name']],
                        'Дата оплаты': [format_date_display(item_data['payment_date'])],
                        'Сумма': [f"{format_currency(item_data['amount'])} ₽"],
                        'Часы': [f"{float(item_data['hours']):.1f}" if pd.notna(item_data['hours']) else '0.0']
                    })
                    ...
                    edited_df = st.data_editor(
                        ...
                        column_config={
                            "ID": st.column_config.NumberColumn("ID", disabled=True),
                            "Услуга": st.column_config.SelectboxColumn("Услуга", options=srv_list),
                            "Дата оплаты": st.column_config.TextColumn("Дата оплаты"),
                            "Сумма": st.column_config.TextColumn("Сумма ₽"),
                            "Часы": st.column_config.TextColumn("Часы")
                        },
                        hide_index=True,
                        use_container_width=True,
                        key=f"item_editor_{item_id}"
                    )

                    # Сохранение изменений
                    if st.button("Сохранить изменения", key=f"save_item_{item_id}"):
                        run_query('''
                            UPDATE order_items
                        ...
                        ''', (
                            edited_df.iloc[0]['Услуга'],
                            parse_date_to_db(edited_df.iloc[0]['Дата оплаты']),
                            parse_currency(edited_df.iloc[0]['Сумма']),
                            float(edited_df.iloc[0]['Часы'].replace(",", ".")) if edited_df.iloc[0]['Часы'] else 0.0,
                            item_id
                        ))
...
# --- 5. ОТЧЁТЫ ---
    ...
    main_query = '''
    SELECT oi.id as item_id, oi.payment_date, oi.amount, oi.hours, oi.service_name,
           o.id as order_id, o.status, o.execution_date,
           c.id as client_id, c.name as client_name, c.first_order_date, g.name as group_name
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.id
    JOIN clients c ON o.client_id = c.id
    LEFT JOIN groups g ON c.group_id = g.id
    ...
    if not df.empty:
        ...
        df_1['Среднее'] = df_1['Среднее'].apply(lambda x: f"{format_currency(x)} ₽")
        ...
        df_new_clients = run_query('''
            SELECT c.name, c.first_order_date, COUNT(oi.id) as count, SUM(oi.amount) as sum
            FROM clients c
        ...
        if not df_new_clients.empty:
            ...
            df_new_clients['sum'] = df_new_clients['sum'].apply(lambda x: f"{format_currency(x)} ₽")
        ...
        df_4 = df.groupby('year').agg(
            Количество=('item_id', 'count'),
            Макс_сумма=('amount', 'max'),
            Мин_сумма=('amount', 'min'),
            Средний_чек=('amount', 'mean'),
            Сумма_год=('amount', 'sum')
        ).reset_index()
        df_4['Средний_месячный'] = df_4['Сумма_год'] / 12
        df_4_chart = df_4[['year', 'Сумма_год']].copy()
        df_4['Макс_сумма'] = df_4['Макс_сумма'].apply(lambda x: f"{format_currency(x)} ₽")
        df_4['Мин_сумма'] = df_4['Мин_сумма'].apply(lambda x: f"{format_currency(x)} ₽")
        df_4['Средний_чек'] = df_4['Средний_чек'].apply(lambda x: f"{format_currency(x)} ₽")
        ...
        df_6['Средний_чек'] = df_6['Средний_чек'].apply(lambda x: f"{format_currency(x)} ₽")
